...
const handleCropIdSearchChange = (e) => {
    setCropIdSearchQuery(e.target.value);
};
...
const handleSaveEdit = async (cropId) => {
    try {
        await axios.put(
            `http://localhost:3000/api/admin/update-crop/${cropId}`,
            newCropData
        );
        toast.success('Crop updated successfully');
        fetchCrops(); // Fetch updated crops list
        setEditCrop(null);
    } catch (error) {
 ...
 const handleSearch = () => {
    ...
    // Update filtered crops to display results
    // setFilteredCrops(result);

    // // Reset input fields
    // setGeneralSearchQuery("");
    // setCropIdSearchQuery("");
    // setFilterType("");
};
        ...
        <div className="p-4">
            <div className="flex mb-4">
                ...
                <input
                    ...
                    value={generalSearchQuery} // State for general search
                ...
                <input
                    ...
                    value={cropIdSearchQuery} // State for crop ID search
        ...
        <div className="bg-white p-6 rounded shadow-lg w-full max-w-lg max-h-[80vh] overflow-y-auto">
            ...
            {Object.keys(newCropData).map((key) => (
                <div key={key} className="mb-4">
                    ...
                    {key === '_id' ? (
                        <div
                            className="p-2 border rounded w-full bg-gray-200 text-gray-700 cursor-not-allowed"
                        >
                            {newCropData[key]}
                        </div>
                    ) : key === 'cropimage1' ? (
            ...
            <div className="mt-6 flex justify-end space-x-4">
                <button
                    onClick={() => handleSaveEdit(editCrop._id)}
