...
const FarmersAndBuyersList = () => {
  ...
  const fetchBuyers = async () => {
    try {
      const response = await axios.get("http://localhost:3000/api/admin/buyers");
          ...
          paginatedData.map((user) => (
            ...
            >
              <div className="text-center">
                <img
                  src={`https://via.placeholder.com/150?text=${user.name.charAt(
                    0
                  )}`}
                  alt={user.name}
                ...
                <p className="text-sm text-gray-600 mb-4">{user.mobileNumber}</p>
              </div>
              <button
                onClick={() =>
                  isBlocked[user._id]
...
export default FarmersAndBuyersList;
